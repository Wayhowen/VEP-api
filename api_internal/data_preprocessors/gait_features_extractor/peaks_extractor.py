...
# read for understanding gaits variations (https://www.science.gov/topicpages/c/cadence+stride+length)
...
class PeaksExtractor:
    ...
    def detect_peaks(self, times, accelerometer_x, accelerometer_y, accelerometer_z) \
        ...
        # Z Axis peaks
        peak_indexes_z, peak_amplitudes_z = self.peaks_detector.detect_peaks(filtered_z)
    ...
    def _filter_data(self, accelerometer_z, times):
        ...
        peak_indexes_z, peak_frequencies_z = self.fft_calculator.calculateFFT(accelerometer_z,
                                                                              self.sampling_rate)
